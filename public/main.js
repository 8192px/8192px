...
viewport.ondblclick = function dblclick(event) {
  ...
  if (event.button == 0) {
    if (viewport.timeout == null) {
      ...
      var bitmap = context.getImageData(x, y, 1, 1);

      bitmap.data[0] = viewport.color[0];
      bitmap.data[1] = viewport.color[1];
      bitmap.data[2] = viewport.color[2];
      bitmap.data[3] = viewport.color[3];
...
viewport.render = function render() {
  ...
  if ('tileX' in viewport && 'tileY' in viewport) {
...
socket.onmessage = function(event) {
  if (typeof event.data == 'string') {
    ...
    if (message.command == 'wait') {
      ...
      var duration = message.timestamp - Date.now();
      viewport.timeout = setTimeout(function tick(then, cooldown) {
        var now = Date.now();
        var time = now - then;
        cooldown = cooldown - time;

        hint.innerHTML = 'Wait... (' + Math.floor(cooldown / 1000) + ')'; 
        ...
        if (cooldown > 0) {
          return (viewport.timeout = setTimeout(tick, 0, now, cooldown));
        }
